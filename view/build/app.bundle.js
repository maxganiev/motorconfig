...
var input_reverseSelection = exports.input_reverseSelection = document.getElementById('input-reverse-selection');
var para_pricePrintout = exports.para_pricePrintout = document.getElementById('para-price-printout');
...
var mask = exports.mask = {
	createMask: function createMask(imgUrl) {
		this.removeMask();

		this.mask = document.createElement('div');
		this.mask.setAttribute('class', 'mask');
		this.mask.innerHTML = '<img src="' + imgUrl + '" class="' + (imgUrl.includes('spinner') ? 'img-status spinner' : 'img-status') + '" alt="temp image" />';
			...
			while (1) {
				switch (_context2.prev = _context2.next) {
					...
					case 35:
						...
						frameSize = optionsSelector.frameSize, brakeType = optionsSelector.brakeType, encoderIsChecked = optionsSelector.encoderIsChecked, ventSystemOptionValue = optionsSelector.ventSystemOptionValue, conicShaftIsChecked = optionsSelector.conicShaftIsChecked, pawType = optionsSelector.pawType;
				...
				switch (_context3.prev = _context3.next) {
					case 0:
						...
						arr_valueToDecode = _global_dom.input_reverseSelection.value.split('/')[1].split('-').slice(1).map(function (val) {
							return (
								...
								val[0] === 'Б'.toUpperCase() && val.length > 1 ? val.slice(1).split('').reduce(function (acc, curr) {
									return [].concat(_toConsumableArray(acc), ['Б' + curr]);
								}, []) : val
							);
						}).flat(1);
						...
						elements.forEach(function (element) {
							var foundIndex = arr_valueToDecode.findIndex(function (val) {
								return (
									//special case for encoder:
									val.includes('N'.toUpperCase()) ? element.getAttribute('data-itemid') === val.slice(0, 1) : //and for the rest:
							...
							switch (element.tagName) {
								case 'OPTION':
									...
									element.value === '-' && setModelDescription('removeData', Array.from(element.parentElement.children).find(function (child) {
										return child.value === '-';
									}).getAttribute('data-itemid'));
								...
								case 'INPUT':
									...
									if (element.checked && element.id === 'checkbox-encoder') {
										...
										var temp = arr_valueToDecode.filter(function (val) {
											return val[0].toUpperCase() === 'N';
										})[0].slice(1);
								...
								case 'BUTTON':
									foundIndex !== -1 ? element.classList.replace('btn-option-non-selected', 'btn-option-selected') : element.classList.replace('btn-option-selected', 'btn-option-non-selected');

									element.classList.contains('btn-option-non-selected') && setModelDescription('removeData', element.getAttribute('data-itemid'));

									setTimeout(function () {
										!element.classList.contains('btn-option-non-selected') && setModelDescription('addData', element.getAttribute('data-itemid'));
									}, 10);
...
function setModelDescription(operationType, htmlElemRef) {
		...
		function clearDesc(listItem_classname) {
			if (document.querySelector('.' + listItem_classname) !== null) {
				...
				Array.from(list.children).forEach(function (child) {
					return child.className.includes(listItem_classname) && child.remove();
				});
			}
		}
		if (htmlElemRef.includes('Б1') || htmlElemRef.includes('Б3') || htmlElemRef.includes('Б5')) {
	...
	function createListItem(optionsConfigPropName, htmlElemRef) {
		var propName = optionsConfigPropName.includes('wiring') || optionsConfigPropName.includes('bearing') ? 'tempDataSensors' : optionsConfigPropName;

		var text = Array.isArray(_base_options_list.optionsConfig[propName]) ? _base_options_list.optionsConfig[propName].find(function (data) {
			return data.id === htmlElemRef;
		}).description : _base_options_list.optionsConfig[propName].description;

		var list = _global_dom.areaRender.lastElementChild.previousElementSibling;

		Array.from(list.children).forEach(function (child) {
			return child.className.includes(optionsConfigPropName) && child.remove();
		});
...
function setModelName() {
	...
	calculateCost: function calculateCost(modelName, pawtype, pricelist, elements) {
		...
		elements.forEach(function (element) {
			switch (element.tagName) {
				case 'BUTTON':
					!element.classList.contains('btn-option-non-selected') && _this2.selectedAttributes.push(element.getAttribute('data-itemid').replace('Б', 'B')) && _this2.selectedElements.push(element);

					break;

				case 'INPUT':
					element.checked && _this2.selectedAttributes.push(element.getAttribute('data-itemid')) && _this2.selectedElements.push(element);
				...
				case 'SELECT':
					if (element.value !== '-') {
						...
						if (element.id === 'selector-climateCat') {
							_this2.selectedAttributes.push(element.value.includes('УХЛ') ? Array.from(element.children).find(function (option) {
								return option.selected === true;
						...
						//separate case for vent selector
								...
								if (Number(Array.from(element.children).find(function (option) {
									...
									//means 54, 55
									Number(document.getElementById('selector-ip').value.slice(2) <= 55) && _this2.selectedAttributes.push(Array.from(element.children).find(function (option) {
										return option.selected === true;
									}).getAttribute('data-itemid') + '_' + document.getElementById('selector-ip').value);
								} else {
									_this2.selectedAttributes.push(Array.from(element.children).find(function (option) {
										return option.selected === true;
									}).getAttribute('data-itemid'));
								}
						...
						_this2.selectedElements.push(Array.from(element.children).find(function (option) {
							return option.selected === true;
						}));
		...
		this.price += Object.keys(pricelist).reduce(function (acc, curr) {
			return _this2.selectedAttributes.indexOf(curr) !== -1 ? acc += window.location.href.includes('manager') ? Number(pricelist[curr]) : Number(pricelist[curr] * 2) : acc;
		}, 0);
	...
	convertCurrency: function () {
		var _ref5 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee4(typeofCurrency) {
			var req, res;
	...
	printResult: function printResult() {
		if (isNaN(this.price)) {
			_global_dom.para_pricePrintout.textContent = 'Стоимость необходимо уточнять.';
		} else {
			_global_dom.para_pricePrintout.innerHTML = this.currency === 'RUB' ? '<strong> \u0421\u0442\u043E\u0438\u043C\u043E\u0441\u0442\u044C \u0438\u0442\u043E\u0433\u043E: </strong>' + new Intl.NumberFormat('ru-RU').format(this.price.toFixed(2)) + ' \u0440\u0443\u0431., \u0432\u043A\u043B\u044E\u0447\u0430\u044F \u041D\u0414\u0421 20%' : ' <strong> \u0421\u0442\u043E\u0438\u043C\u043E\u0441\u0442\u044C \u0438\u0442\u043E\u0433\u043E: </strong>' + new Intl.NumberFormat('kk-KK').format(this.price.toFixed(2)) + ' \u0442\u043D\u0433.';
		}
	},

	expandPricelist: function expandPricelist() {}
};
...
var _selectFunctions = __webpack_require__(134);
...
function globeEvHandler() {
	...
	_global_dom.btn.btn_currencyConverter.onclick = function (e) {
		e.preventDefault();

		_selectFunctions.motorCost.convertCurrency(e.target.getAttribute('data-itemid'));
		...
		e.target.style.background = 'url("/image/catalog/adchr/' + (e.target.getAttribute('data-itemid') === 'KZT' ? 'kz' : 'ru') + '.svg") center center/cover';
	};
	...
	_global_dom.selectorBrakes.addEventListener('change', function (e) {
		...
		if (e.target.value !== '-') {
			(0, _selectFunctions.setModelDescription)('addData', selOptionId);
		} else {
	...
	document.body.addEventListener('change', function (e) {
		...
		if (e.target.id === 'selector-ip') {
			...
			(0, _selectFunctions.setModelDescription)('addData', _selOptionId);
			(0, _selectFunctions.fillUpgradesChart)();
	...
	document.body.addEventListener('click', function (e) {
		...
		if (e.target.id.includes('btn-options-sensors-id')) {
			if (Array.from(e.target.classList).some(function (className) {
				...
				(0, _selectFunctions.setModelDescription)('addData', Array.from(e.target.classList).find(function (cl) {
				...
				(0, _selectFunctions.setModelDescription)('removeData', Array.from(e.target.classList).find(function (cl) {
					return cl.includes('Б');
				}));
			}
			(0, _selectFunctions.getOptions)(null);
	...
	_global_dom.input_reverseSelection.onkeydown = function (e) {
		...
		_global_dom.selectorPower.value = _global_dom.selectorRpm.value = '-';

		var permitted1 = ['v', 'a', 'c', 'x', 'z', 'r'];
		var permitted2 = ['Tab', 'Control', 'Alt', 'Shift', 'ArrowLeft', 'ArrowRight', 'Home', 'End', 'Backspace'];
...
function fillExtraOptions() {
	...
	if (_global_dom.areaSelection.children.length <= 6) {
		...
		createCheckBoxes(_global_dom.areaSelection, 'checkbox-vibrosensors', vibroSensors.id, !vibroSensors.selectable, vibroSensors.type, null);

		//чекбокс для выбора антиконденсатного подогрева:
		createCheckBoxes(_global_dom.areaSelection, 'checkbox-antiCondenseHeater', antiCondensingHeater.id, !antiCondensingHeater.selectable, antiCondensingHeater.type, null);

		//чекбокс для выбора токоизолированного подшипника:
		createCheckBoxes(_global_dom.areaSelection, 'checkbox-currentInsulatingBearing', currentInsulatingBearing.id, !currentInsulatingBearing.selectable, currentInsulatingBearing.type, currentInsulatingBearing.checked);
		...
		createCheckBoxes(_global_dom.areaSelection, 'checkbox-package', 'pkg', false, 'Дополнительная упаковка оборудования', false);
	}
...
function createSelects(parentElem, selectId, srcDataToFillOptions) {
	var listItem = selectId !== 'selector-encoderVoltage' && selectId !== 'selector-outputSignal' ? document.createElement('li') : parentElem;
...
function showWarning() {
	...
	if (_selectFunctions.optionsSelector.frameSize >= 200 && !checkboxCurrentInsulatingBearing.checked && selectorImportBearings.value !== 'Передний и задний шариковые подшипники (производства SKF/NSK/KOYO/FAG)') {
		...
		checkboxCurrentInsulatingBearing.parentElement.insertAdjacentHTML('beforeend', '<p class="warning"> ' + _base_options_list.optionsConfig.currentInsulatingBearing.warning + ' </p>');
	} else {
		Array.from(checkboxCurrentInsulatingBearing.parentElement.childNodes).forEach(function (node, index) {
			return index === 2 && node.remove();
		});
